layui.use(['upload','layer','layedit'], function(){
  var upload = layui.upload,layer = parent.layer === undefined ? layui.layer : parent.layer;
    ...
    upload.render({
        elem: '#test4',
        url: "<?=yii\helpers\Url::to(['/tools/upload?name=qingrui'])?>",
        done: function(res){
            if(res.code==200){
                //修改上传成功后需要修改的地方
                $("#qingcase-case_img").val(res.data);
                $(".case_img").attr('src',res.data);
                layer.msg("上传成功");
            }else{
                layer.msg("上传失败");
            }
    ...
    layedit.set({
        uploadImage: {
            url: "<?=yii\helpers\Url::to(['/tools/uploadedit?name=qingrui'])?>",
            type: 'post',
            done: function (res) {
                // 如果上传失败
                if (res.code > 0) {
    ...
    layedit.build('case_content'); //建立编辑器
});